...
func ReadMac(reader *bytes.Reader) string {
	mac_byte := make([]byte, 6)
	reader.Read(mac_byte)
	var mac_string string
	for _, value := range mac_byte {
		mac_string += hex_table[value]
	}

	return mac_string
}

func ReadMacInt(reader *bytes.Reader) uint64 {
	mac_byte := make([]byte, 6)
	reader.Read(mac_byte)
	mac := []byte{0, 0}
